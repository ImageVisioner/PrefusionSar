function [sMap,WB,Cmax,i_mean2] = weight_spd(I,p,r)
R=2*r+1;
...
i_mean2= zeros(size(I,1),size(I,2),N); %% 平均值
for i = 1:N
    ...
    %     WB(:,:,i)=2/pi*atan(30*i_mean2(:,:,i)).*imgradient(i_mean2(:,:,i));
    %   WB(:,:,i)=2/pi*atan(30*i_mean2(:,:,i));
    WB(:,:,i)=mean_fun(i_mean2(:,:,i),2).*imgradient(i_mean2(:,:,i));
    %  WB(:,:,i)=i_mean2(:,:,i).*imgradient(i_mean2(:,:,i));
end
